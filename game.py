...
class Game:
    def __init__(self, height, width, gui):
        ...
        self.fpp_yaw = -90.0  # Start looking forward (negative Z)
        self.fpp_pitch = 0.0  # Start looking horizontally
...
#             # Create a minimap window to show the direction to the target planet.
            ...
            if self.objects.get("transporter") is not None:
                ...
                if inputs.get("R_CLICK") is True:
                    # Switch to FPP mode
                    if self.active_camera != self.fpp_camera:
                        # Set initial orientation when switching to FPP
                        forward_spaceship = transporter.properties["orientation"] @ np.array([0, 0, -1], dtype=np.float32)
                        up_spaceship = transporter.properties["orientation"] @ np.array([0, 1, 0], dtype=np.float32)
                        
                        # Reset yaw and pitch to match the ship's current orientation
                        self.fpp_yaw = -90.0
                        self.fpp_pitch = 0.0
                        
                        # Enable cursor lock for mouse control
                        self.gui.window.lock_cursor = True
                        glfw.set_input_mode(self.gui.window, glfw.CURSOR, glfw.CURSOR_DISABLED)
                        
                    # Now update FPP camera position based on ship
                    forward_offset = 10.0
                    forward_spaceship = transporter.properties["orientation"] @ np.array([0, 0, -1], dtype=np.float32)
                    self.fpp_camera.position = transporter.properties["position"] + forward_spaceship * forward_offset
                    
                    # Handle mouse movement for camera rotation
                    mouse_delta = inputs.get("mouseDelta", [0, 0])
                    self.fpp_yaw += mouse_delta[0] * self.mouse_sensitivity
                    self.fpp_pitch -= mouse_delta[1] * self.mouse_sensitivity
                    
                    # Restrict pitch to avoid flipping
                    if self.fpp_pitch > 89.0:
                        self.fpp_pitch = 89.0
                    if self.fpp_pitch < -89.0:
                        self.fpp_pitch = -89.0
                    ...
                    direction = np.array([
                        np.cos(np.radians(self.fpp_yaw)) * np.cos(np.radians(self.fpp_pitch)),
                        np.sin(np.radians(self.fpp_pitch)),
                        np.sin(np.radians(self.fpp_yaw)) * np.cos(np.radians(self.fpp_pitch))
                    ], dtype=np.float32)
                    ...
                    self.fpp_camera.lookAt = self.fpp_camera.position + direction
                    self.fpp_camera.up = np.array([0, 1, 0], dtype=np.float32)
                    self.active_camera = self.fpp_camera
                else:
                    # Switch to third person mode
                    if self.active_camera == self.fpp_camera:
                        # Show cursor again when switching back to normal camera
                        self.gui.window.lock_cursor = False
                        glfw.set_input_mode(self.gui.window, glfw.CURSOR, glfw.CURSOR_NORMAL)
            ...
            if "lasers" in self.objects and "pirates" in self.objects:
                # Lists to track objects that need to be removed
                lasers_to_remove = []
                pirates_to_remove = []
                ...
                for laser_obj in self.objects["lasers"]:
                    if "velocity" in laser_obj.properties:
                        ...
                        laser_obj.properties["position"] += laser_obj.properties["velocity"] * delta
                        laser_pos = laser_obj.properties["position"]
                        
                        # Check if laser is out of bounds
                        if np.linalg.norm(laser_pos) > 6000:
                            lasers_to_remove.append(laser_obj)
                            continue
                        ...
                        for pirate_obj in self.objects["pirates"]:
                            pirate_pos = pirate_obj.properties["position"]
                            # Laser hit detection with a reasonable distance threshold
                            hit_threshold = 1500.0  # Adjust based on model size
                            if np.linalg.norm(laser_pos - pirate_pos) < hit_threshold:
                                # Mark both laser and pirate for removal
                                if pirate_obj not in pirates_to_remove:
                                    pirates_to_remove.append(pirate_obj)
                                if laser_obj not in lasers_to_remove:
                                    lasers_to_remove.append(laser_obj)
                                print("Laser hit a pirate!")
                                break
                ...
                for laser_obj in lasers_to_remove:
                    if laser_obj in self.objects["lasers"]:
                        self.objects["lasers"].remove(laser_obj)
                
                # Remove and respawn destroyed pirates
                for pirate_obj in pirates_to_remove:
                    if pirate_obj in self.objects["pirates"]:
                        self.objects["pirates"].remove(pirate_obj)
                        
                        # Spawn a replacement pirate at a random location
                        ship_pos = self.objects["transporter"].properties["position"]
                        min_distance = 300  # Minimum spawn distance from player
                        ...
                        while not valid_spawn:
                            ...
                            if np.linalg.norm(pos - ship_pos) > min_distance:
                                valid_spawn = True
                        ...
                        pirate["scale"] = np.array([scale_val, scale_val, scale_val], dtype=np.float32)
                        self.objects["pirates"].append(Object(None, self.shaders[0], pirate))
                        print("Spawned replacement pirate")
                
                # Update remaining pirates
                ship_pos = self.objects["transporter"].properties["position"]
                pirate_speed = 50.0
                for pirate_obj in self.objects["pirates"]:
                    pirate_pos = pirate_obj.properties["position"]
